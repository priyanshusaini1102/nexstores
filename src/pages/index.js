...
import { ChevronDownIcon, EditActiveIcon, EditInactiveIcon } from '@heroicons/react/20/solid'
...
export default function Home({products}) {
  ...
  useEffect(()=>{
    const filtered=products.filter((product)=> product.type.includes(selectedCategory==='All' ? '' : selectedCategory));
  ...
  return (
    <>
      ...
      <Menu as="div" className="relative inline-block text-left">
        ...
        >
          <Menu.Items className="absolute z-10 left-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            {categories.map((c)=>(<div key={c} className="px-1 py-1 ">
              <Menu.Item>
                {({ active }) => (
                  <button
      ...
      {
        filteredProducts?.map((product)=> <Link key={product?.id} href={`./product/${product?.id}`}><ProductCard key={product?.id} product={product}/></Link>)
      }
      </div>
      </div>
